...
const char* usage ="./myCapCamera -c=cameraIndx -w=width -h=height -s=saveFileName\n"
                   "./myCapCamera -c=0 -w=800 -h=600 -s=../data/saveFileName.avi\n";
...
{
    cv::CommandLineParser parser(argc, argv, "{help||}{c|0|camera index}{w|1280|width}{h|720|height}{s|../data/saveFileName.avi|save file name}{f|false|remap flap}");
    if(argc<2)
    {
        help();
        return 0;
    ...
    int videoIndex = parser.get<int>("c");
    int imgW = parser.get<int>("w");
    int imgH = parser.get<int>("h");
    std::string saveFileName = parser.get<string>("s");
    bool isRemap = parser.get<bool>("f");

    VideoCapture capture(videoIndex);//如果是笔记本，0打开的是自带的摄像头，1 打开外接的相机
    double rate = 25.0;//视频的帧率  
    Size videoSize(imgW,imgH);

    int isColor=1;
    Point centerImage(videoSize.width/2, videoSize.height/2);
    Point center(videoSize.width/2, videoSize.height/2);
...
//        cvSize(frame->width, frame->height), isColol);
	...
	flip(frame,frame,1);
        ...
        circle(showFrame,center,15,Scalar(255,0,0),2);
